...
class FinancialFlowForm
{
                ...
                Section::make('بيانات المستند')
                    ...
                    ->schema([
                        Grid::make(2)->schema([
                            Select::make('type')
                                ->label('نوع التدفق')
                                ->options(fn (Get $get): array => FinancialFlowType::selectOptions($get('type')))
    ...
    private static function selectedType(Get $get): ?FinancialFlowType
    {
        $type = $get('type');

        return filled($type) ? FinancialFlowType::cached()->firstWhere('slug', $type) : null;
    }
}
